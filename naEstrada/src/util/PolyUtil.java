...
public class PolyUtil {
    ...
    public static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // LatLng.getLat() and .getLng() are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(new LatLng(lastPoint.getLat() + OFFSET, lastPoint.getLng() + OFFSET));
        }
    ...
    public static double distanceToLine(final LatLng p, final LatLng start, final LatLng end) {
        ...
        LatLng sa = new LatLng(p.getLat() - start.getLat(), p.getLng() - start.getLng());
        LatLng sb = new LatLng(u * (end.getLat() - start.getLat()), u * (end.getLng() - start.getLng()));
        System.out.println("sa: " + sa.toString() + "sb: " + sb.toString());
	...
	public static LatLng closestPointOnSegment(LatLng p, LatLng p1, LatLng p2) {
		double x = p1.getLng();
		double y = p1.getLat();
		double dx = p2.getLng() - x;
		double dy = p2.getLat() - y;
		double dot = dx * dx + dy * dy;
		double t;

		if (dot > 0) {
			t = ((p.getLng() - x) * dx + (p.getLat() - y) * dy) / dot;
	...
	public static LatLng closestPointOnPath(LatLng p, List<LatLng> path) {
		LatLng closest = null;
		...
		for (int i = 0; i + 1 < path.size(); i++) {
			...
			LatLng candidata = closestPointOnSegment(p, p1, p2);
			double distancia = SphericalUtil.computeDistanceBetween(p, candidata);
